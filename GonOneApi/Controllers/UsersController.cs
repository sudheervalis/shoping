...
namespace GonOneApi.Controllers
{
    ...
    [ApiController]
        ...
        public class UsersController : BaseApiController
        {
            ...
            [HttpPost]
            [Route("login")]
            public async Task<IActionResult> Login(MLoginRequest loginRequest)
            {
                if (loginRequest == null || string.IsNullOrEmpty(loginRequest.Email) || string.IsNullOrEmpty(loginRequest.Password))
                {
                    return BadRequest(new MTokenResponse
                    {
            ...
            [HttpPost]
            [Route("refresh_token")]
            public async Task<IActionResult> RefreshToken(MRefreshTokenRequest refreshTokenRequest)
            {
                ...
                return Ok(new MTokenResponse { AccessToken = tokenResponse.Item1, RefreshToken = tokenResponse.Item2 });
            }

            [HttpPost]
            [Route("signup")]
            public async Task<IActionResult> Signup(MSignupRequest signupRequest)
            {
